...
LONG = (
    b"Cabin baggage is one bag up to eight kilograms, plus one personal "
    b"item that fits under the seat in front of you. "
) * 12
SHORT = b"# Probe\n\nOne bag.\n"
...
def main() -> int:
    with httpx.Client(
        ...
        try:
            ...
            print(
                f"backend is not running at {BASE_URL} -- start it with 'make dev'",
                file=sys.stderr,
            )
            return 1
        ...
        check(
            "status is indexed",
            body.get("status") == "indexed",
            str(body.get("status")),
        )
        check(
            "chunks were produced",
            body.get("chunk_count", 0) > 1,
        ...
        check(
            "identical bytes do no work",
            again.get("status") == "unchanged",
        ...
        check(
            "chunk count follows the new content",
        ...
        check(
            "everything uploaded here is gone", not remaining, f"{len(remaining)} left"
        )
